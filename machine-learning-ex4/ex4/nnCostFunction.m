...
%     Y = Y';
    for i = 1:m
        ...
        output = sigmoid([1 sigmoid(x(i, :) * Theta1')] * Theta2');
        
        Deb = (-1*y_i)' * log(output') - (1 - y_i)' * log(1 - output');
...
%     J = J + lambda / 2 * (sum(sum(Theta1.^2)) + sum(sum(Theta2.^2)) - sum(Theta1(:,1).^2) - sum(Theta2(:,1).^2));
